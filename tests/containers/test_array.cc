...
TEST(ArrayTest, to_array) {
  auto arr = memory::to_array({1, 2, 3, 4, 5, 6, 7});
...
TEST(ArrayTest, to_array_custom_order) {
  auto arr =
      memory::to_array({1, 2, 3, 4, 5, 6, 7},
                   std::integer_sequence<std::size_t, 6u, 5u, 4u, 3u, 2u, 1u, 0u>{});
...
TEST(ArrayTest, to_array_copy) {
  auto arr =
      memory::to_array({1, 2, 3, 4, 5, 6, 7},
                   std::integer_sequence<std::size_t, 6u, 6u, 6u, 6u, 6u, 6u, 6u>{});

  for (int i = 0; i < arr.size(); i++) ASSERT_EQ(arr[i], 7);
}

TEST(ArrayTest, to_array_constexpr) {
  constexpr auto arr = memory::to_array({1, 2, 3, 4, 5, 6, 7});
